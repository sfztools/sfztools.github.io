...
## @brief Returns a html `img` code fragment to display the selected emoji.
#
# Note that ATM only the required emojis are saved in assets directory.
#
# @param key  The name of the emoji (e.g.: smile) without colons.
# @param path The relative path from the page is used to the emojis directory.
#
def get_emoji(key: str, path: str):
  global _emojis
  if key in _emojis:
    return \
"""
<img class=\"emoji\" title=\":{}:\" alt=\":{}:\"
  src=\"{}/{}.png\" height=\"20\" width=\"20\">
""".format(key, key, path, _emojis[key])
